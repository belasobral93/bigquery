...
select 
    ...
    line_item.tax_rate,
    ((line_item.extended_price * (1 - line_item.discount_percentage) + (-1 * line_item.extended_price * line_item.discount_percentage)) * line_item.tax_rate) as item_tax_amount,
    (
        line_item.extended_price * (1 - line_item.discount_percentage) + 
        (-1 * line_item.extended_price * line_item.discount_percentage) + 
        ((line_item.extended_price * (1 - line_item.discount_percentage) + (-1 * line_item.extended_price * line_item.discount_percentage)) * line_item.tax_rate)
